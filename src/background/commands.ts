...
const KNOWN: Record<CommandName, (cmd: Command) => Promise<unknown>> = {
  ...
  check_registry_updates: (cmd) => tpl.checkRegistryUpdates(cmd),
...
export function buildGuideFromCmd(profile: ProfileInfo | undefined, hostPort: number | undefined, hostToken?: string): string {
  ...
  return [
    ...
    "",
    "```",
    '{"type":"command","name":"list_templates","args":{},"requestId":"0","authToken":"' + auth + '"}',
    '{"type":"command","name":"search_templates","args":{"query":"<关键词>"},"requestId":"1","authToken":"' + auth + '"}',
    '{"type":"command","name":"run_template","args":{"id":"chatgpt-ask","params":{"prompt":"<问题>"}},"requestId":"2","authToken":"' + auth + '"}',
    '{"type":"command","name":"run_template","args":{"id":"chatgpt-ask","params":{"prompt":"<追问>","conversationUrl":"<上次返回的 url>"}},"requestId":"3","authToken":"' + auth + '"}',
    "",
    "追问（同一会话续聊）：AI 模板（chatgpt-ask/gemini-ask/deepseek-ask）每次返回 url；把它作为 conversationUrl 传入即在同一会话追问，上下文保持。URL 每轮会变——始终用最新一次返回的 url。",
    "```",
    "",
    "注意：除 ping/reload 外所有命令都要带 authToken（如上）。全部模板随扩展发布、开箱即用（`list_templates` 可见，共 30+ 个，带 tags 分类）；GitHub Registry 仅用于发现更新/新增，`check_registry_updates` 查看后按需 `install_template` 覆盖，用 `search_templates`/`get_template_detail` 检索详情。",
    "```",
    "",
    "没有现成模板时，按下面三步手写：",
    "",
    ...
    "| `open_space` | `{name?,url?,focus?}` | 创建并选中当前 Agent 独占的窗口空间。**默认后台**：不聚焦且最小化，不打扰用户桌面；显式 `focus:true`/`keepVisible:true`/`visible:true`/`background:false` 才前台最大化 |",
    ...
    "| `open_tab` | `{url,focus?}` | 在当前 Task Space 内开新标签，返回 `{tabId,spaceId}`；默认后台不抢激活，显式 `focus:true` 才前台 |",
    "| `close_tab` | `{tabId}` | 关标签 |",
    "| `switch_tab` | `{tabId,visible?}` | 登记/切换目标标签；默认不聚焦，显式 `visible:true` 才前台激活 |",
    "| `ensure_visible` | `{tabId?,state?}` | 显式前台路径：恢复目标窗口为 normal 并聚焦，降低最小化/后台渲染节流 |",
    ...
    "| `run_template` | `{id, params?, focus?}` | 一键跑流程模版(省 token); **默认后台执行**（不恢复/不聚焦窗口），显式 `focus:true`/`keepVisible:true`/`visible:true`/`background:false` 才前台运行（AI 聊天类模板内部已显式声明前台）; 聊天模版续问传 conversationUrl(上次返回的 url) 校验仍指向同一会话; 失灵返回结构化失败上下文 |",
    ...
    "- **failback 现场重写模板的最小 JSON**（import_template {content:<JSON 字符串>} 安装）：{id, name, version:\"1.0.0\", description, category, inputs:[{name,type,required}], steps:\"commands\", body:[{name:\"open_tab\", args:{url:\"$url\"}}, ...], discovery:{intents, keywords, outputs, risk}} —— 完整字段参考本机仓库 registry/templates/*/template.json；body 步骤名必须在命令白名单内。",    "- **AI 聊天页是流式渲染**：`snapshot`/`js` 返回的都是瞬时快照；用 `wait_dom_idle` 等回答写完再收集，并核对结尾是否为自然句子——「停止按钮消失」只是必要非充分条件。",
    "- **读当前会话的 AI 回复**：L0 快照不含消息正文。wait_dom_idle 后用 readText 按选择器读回复文本：ChatGPT 用 `.markdown`，DeepSeek 用 `.ds-markdown:last-of-type`。",
    ...
    "- **host 断链且 ping 不通（如长任务中宿主重启/换端口）**：`export_guide` 此刻是死路（它本身需要连上宿主）。恢复方式：读 `~/Library/Application Support/BrowserPilot/sessions/`（macOS）或 `%LOCALAPPDATA%\\BrowserPilot\\sessions\\`（Windows）下最新的 `<port>.json`，里面有最新的 port 和 authToken；或重启 Chrome 让 SW 自动拉起 host。",
